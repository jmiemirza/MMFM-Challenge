...
def call_llava_engine_df(args, sample, model, tokenizer=None, processor=None):
    ...
    if ('shots' in sample) and (sample['shots'] is not None):
        for s in sample['shots']:
            conv.append_message(conv.roles[0],  s['final_input_prompt'])
            conv.append_message(conv.roles[1], s['answer'])

    conv.append_message(conv.roles[0], prompt)
    conv.append_message(conv.roles[1], None)
    prompt = conv.get_prompt()
    input_ids = tokenizer_image_token((prompt[0] if isinstance(prompt, tuple) else prompt), tokenizer, IMAGE_TOKEN_INDEX, return_tensors='pt').unsqueeze(0).cuda()
    image = sample['image']
